    ...
    while True:
        ...
        for s in range(1, nstates + 1):
            ...
            for action, action_prob in enumerate(policy[s - 1]):
                ...
                if s not in terminal_ind:
                    v += action_prob * (rewards[s - 1] +  gamma * np.dot(transition_models[action].loc[s, :].values, V))
                else:
                    v = rewards[s - 1]
...
def lookForPolicy(s, V, transition_models, rewards, gamma=0.9):
...
def calc_policy(policy):
    nstates = world.get_nstates()
    nactions = world.get_nactions()
    P = np.zeros((nstates, 1))
    for row in range(nstates):
        for col in range(nactions):
            if policy[row, col] == 1:
                P[row] = col + 1
    return P
...
def policy_improvement(world, transition_models, rewards, gamma=0.9):
    ...
    while True:
        ...
        for s in range(1, nstates + 1):
            ...
            action_values = lookForPolicy(s, V, transition_models, rewards, gamma)
